...
Default_Tool_bat = program_folder + '\\resources\default\empty_tool_database.dat'# Path to default tool database
Generated_Tool_bat = program_folder + '\spreadsheets\generated_tool_database.dat'# Path to generated tool database
try:
    Tools_table = sys.argv[1]
    print("\n-------- Excel File from GUI Input --------\n")
except:
    Tools_table = program_folder + '\spreadsheets\Tool_library.xlsx'  # Path to read desired tool Excel file
    print("\n-------- Excel File from Script --------\n")
print('Input File: '+str(Tools_table)+'\n')
...
def convert_to_dat():
    ...
    with open(Default_Tool_bat, 'r') as file: # Open empty tool database file
        text = file.read().split('\n') # Make "text" variable from file content
        #print(type(text))
    ...
    with open(Generated_Tool_bat,'w') as outfile:
        outfile.write(A)
    print('Output path: '+ str(Generated_Tool_bat))
    print("\n-------- .dat File Created --------\n")
